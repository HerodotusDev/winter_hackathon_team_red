...
impl StorageMptHandler {
    ...
    pub async fn fake_storage_slot(&self, address: alloy::primitives::Address, storage_slot: H256, block_number: u64, new_value: U256) -> Result<(), EthTrieError> {
        ...
        let account_proof = self.parse_account_proof(address, block_number, proof).await?;
        let account_payload = self.verify_account_proof(account_proof.clone())?;

        let res = self.verify_storage_proof(storage_proof.clone())?;
        println!("old_storage_hash: {:?}", hex::encode(storage_proof.storage_hash.as_bytes()));
        ...
        println!("new_storage_hash: {:?}", hex::encode(new_storage_proof.root.as_bytes()));
        println!("new_value: {:?}", hex::encode(new_res.clone()));
        println!("______________________________________________________");

        println!("old_state_root: {:?}", hex::encode(account_proof.root.as_bytes()));
        println!("old_account_payload: {:?}", hex::encode(account_payload.clone()));
        println!("______________________________________________________");

        let new_account_payload = self.update_account_storage_root(account_payload, new_storage_proof.root)?;
        let new_account_proof = self.update_proof_auto(account_proof.into(), new_account_payload)?;
        let new_account_res = self.verify_account_proof(new_account_proof.clone().into())?;
        println!("new_state_root: {:?}", hex::encode(new_account_proof.root.as_bytes()));
        println!("new_account_payload: {:?}", hex::encode(new_account_res.clone()));
    ...
    async fn parse_account_proof(&self, address: alloy::primitives::Address, block_number: u64, proof: EIP1186AccountProofResponse) -> Result<AccountProof, EthTrieError> {
    ...
    pub fn verify_storage_proof(&self, proof: StorageProof) -> Result<Vec<u8>, EthTrieError> {
        ...
        let result = trie.verify_proof(proof.storage_hash, proof.storage_slot.as_bytes(), proof.storage_proof).unwrap().unwrap();
        Ok(result)
    }
    ...
    fn update_account_storage_root(&self, account_rlp: Vec<u8>, new_storage_hash: H256) -> Result<Vec<u8>, EthTrieError> {
        let mut account = Account::decode(&mut account_rlp.as_slice()).unwrap();
    ...
    fn update_storage_slot(&self, storage_rlp: Vec<u8>, new_value: U256) -> Result<Vec<u8>, EthTrieError> {
    ...
    fn update_proof_auto(&self, proof: MptProof, new_leaf: Vec<u8>) -> Result<MptProof, EthTrieError> {
